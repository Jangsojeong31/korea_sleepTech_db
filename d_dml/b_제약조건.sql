...
/*
    ...
    : 테이블의 레코드(행)을 고유하게 구분할 수 있는 식별자 역할
    ...
    cf) 유일성 제약: 하나의 테이블 당 하나의 기본키만 지정 가능
		>> 테이블의 특성을 가장 잘 반영한 열 선택을 권장
			ex) members 테이블(member_id), books 테이블(isbn | book_id)와 같이 
				PK 컬럼을 따로 생성 권장
...
-- values
-- 	(1, '조승범', 'C전공');
# Error Code: Duplicate entry '1' for key 'students PRIMARY'
# 기본키는 중복 데이터 불가
...
# 1) '기존 테이블의' 제약 조건 삭제
# 기본 키 제약 조건 삭제 시, not null 에 대한 조건은 사라지지 않는다.
...
/*
    ...
    : 외래 키가 설정된 열은 반드시 다른 테이블의 기본키와 연결!!
...
# >> 고객이 실제로 존재하는 지 확인, 고객과 주문간의 관계를 명시 
...
create table `orders` (
    ...
    member_id int,
    # 외래키 지정 방식
    # foreign key (참조컬럼, 해당 테이블의 칼럼) references 기본테이블(기본컬럼)
    foreign key (member_id) references `members` (member_id)
);

# 유효하지 않은 고객 참조 시도 - 오류: Cannot add or update a child row: a foreign key constraint fails ~
-- insert into `orders`
-- values 
-- 	(1, '2025-04-11', 123);
