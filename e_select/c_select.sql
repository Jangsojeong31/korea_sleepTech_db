...
/*
	1. order by 
        ...
        cf) 오름차순: 알파벳(a), 자음(ㄱ), 날짜(오래된 순, 과거), 숫자(작은 데이터)부터 출력
        
*/

select * from `members`; # 데이터 삽입 순서대로 정렬 (auto_increment 값에 따라 정렬)
...
/*
    ...
    cf) 첫 번째 행이 기본값 0 (index 번호 체계와 동일)
...
# 지역별 평균 포인트 계산
select area_code, avg(points) as memeber_point
from `members`
group by area_code;
    
/*
	5. having 
    : group by 함께 사용, 그룹화된! 결과에 대한 조건을 지정
    - where 조건과 사용 식은 유사하지만, 그룹화된 데이터에 대해 조건을 지정
*/
select grade, count(*) as member_count
from `members`
group by grade
